...
static CAN_TxHeaderTypeDef Can2_Lboard,Can2_Rboard;
...
/**********************************************************************************************************
 *函 数 名:LC_BoardCan2Send
 *功能说明: 与左c板的板间通信,传输云台角度设定值
 *形    参: Lpitch,Lyaw,
 *返 回 值: 无
 **********************************************************************************************************/
static uint8_t L_Cboard_send_data[8];
void LC_BoardCan2Send(float Lpitch,float Lyaw)
{
	memcpy(L_Cboard_send_data,&Lpitch,4);
	memcpy(L_Cboard_send_data+4,&Lyaw,4);
	uint32_t send_mail_box;
	Can2_Lboard.StdId=0x090;//stdid
	Can2_Lboard.IDE=CAN_ID_STD;
	Can2_Lboard.RTR=CAN_RTR_DATA;
	Can2_Lboard.DLC=0x08;
//	Lpitch = LIMIT_MAX_MIN(Lpitch, 30000, -30000);
//	Lyaw = LIMIT_MAX_MIN(Lyaw, 1600, -1600);//数据需要查手册
	HAL_CAN_AddTxMessage(&hcan2, &Can2_Lboard, L_Cboard_send_data, &send_mail_box);
}

/**********************************************************************************************************
 *函 数 名:LC_BoardCan2Send
 *功能说明: 与左c板的板间通信,传输左云台的状态
 *形    参: Lfri_mode,Lbodan_mode,LGimbal_mode
 *返 回 值: 无
 **********************************************************************************************************/
static uint8_t L_Cboard_send_data2[8];
void LC_BoardCan2Send2(Gimbal_mode_t myGimbal)
{
	uint32_t send_mail_box;
	Can2_Lboard.StdId=0x091;//stdid
	Can2_Lboard.IDE=CAN_ID_STD;
	Can2_Lboard.RTR=CAN_RTR_DATA;
	Can2_Lboard.DLC=0x08;
//	Lpitch = LIMIT_MAX_MIN(Lpitch, 30000, -30000);
//	Lyaw = LIMIT_MAX_MIN(Lyaw, 1600, -1600);//数据需要查手册
	L_Cboard_send_data[0] = (unsigned char)(myGimbal.fri_mode);//摩擦轮是否开启
	L_Cboard_send_data[1] = (unsigned char)(myGimbal.bodan_mode);//拨蛋电机转动模式
	L_Cboard_send_data[2] = (unsigned char)(myGimbal.AutoFire_Flag);//左云台开火模式
	L_Cboard_send_data[3] = (unsigned char)(myGimbal.Laser_Flag);//激光
	L_Cboard_send_data[4] = (unsigned char)(myGimbal.Gimbal_Flag);//辅瞄
	L_Cboard_send_data[5] = (unsigned char)(myGimbal.Freq_state);//射频
	L_Cboard_send_data[6] = 0;
	L_Cboard_send_data[7] = 0;//暂时还没想到需要传输什么数据
	HAL_CAN_AddTxMessage(&hcan2, &Can2_Lboard, L_Cboard_send_data2, &send_mail_box);
}

/**********************************************************************************************************
 *函 数 名:RC_BoardCan2Send
 *功能说明: 右c板的板间通信，传输云台角度
 *形    参: Rpitch,Ryaw
 *返 回 值: 无
 **********************************************************************************************************/
static uint8_t R_Cboard_send_data[8];
void RC_BoardCan2Send(float Rpitch,float Ryaw)
{
	memcpy(R_Cboard_send_data,&Rpitch,4);
	memcpy(R_Cboard_send_data+4,&Ryaw,4);
	uint32_t send_mail_box;
	Can2_Rboard.StdId=0x092;//stdid暂定为0x092
	Can2_Rboard.IDE=CAN_ID_STD;
	Can2_Rboard.RTR=CAN_RTR_DATA;
	Can2_Rboard.DLC=0x08;
//	Rpitch = LIMIT_MAX_MIN(Rpitch, 30000, -30000);
//	Ryaw = LIMIT_MAX_MIN(Ryaw, 30000, -30000);//6020最大can输入
	
	HAL_CAN_AddTxMessage(&hcan2, &Can2_Rboard, R_Cboard_send_data, &send_mail_box);
}

/**********************************************************************************************************
 *函 数 名:RC_BoardCan2Send2
 *功能说明: 右c板的板间通信,传输右云台状态
 *形    参: Rpitch,Ryaw,Rfri_mode,Rbodan_mode,RGimbal_mode
 *返 回 值: 无
 **********************************************************************************************************/
static uint8_t R_Cboard_send_data2[8];
void RC_BoardCan2Send2(Gimbal_mode_t myGimbal)
{
	uint32_t send_mail_box;
	Can2_Rboard.StdId=0x093;//stdid暂定为0x093
	Can2_Rboard.IDE=CAN_ID_STD;
	Can2_Rboard.RTR=CAN_RTR_DATA;
	Can2_Rboard.DLC=0x08;
	R_Cboard_send_data[0] = (unsigned char)(myGimbal.fri_mode);//摩擦轮是否开启
	R_Cboard_send_data[1] = (unsigned char)(myGimbal.bodan_mode);//拨蛋电机转动模式
	R_Cboard_send_data[2] = (unsigned char)(myGimbal.AutoFire_Flag);//左云台开火模式
	R_Cboard_send_data[3] = (unsigned char)(myGimbal.Laser_Flag);//激光
	R_Cboard_send_data[4] = (unsigned char)(myGimbal.Gimbal_Flag);//辅瞄
	R_Cboard_send_data[5] = (unsigned char)(myGimbal.Freq_state);//射频
	R_Cboard_send_data[6] = 0;
	R_Cboard_send_data[7] = 0;//暂时还没想到需要传输什么数据
	HAL_CAN_AddTxMessage(&hcan2, &Can2_Rboard, R_Cboard_send_data2, &send_mail_box);
}
