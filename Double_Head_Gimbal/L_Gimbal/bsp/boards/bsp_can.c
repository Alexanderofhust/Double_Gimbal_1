...
//char Lfri_mode=0,Lbodan_mode=0,AutoFire_Flag=0,Laser_Flag=0,Gimbal_Flag=0,Freq_state=0;
extern Gimbal_mode_t L_Gimbal;
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	{      // 处理CAN1的数据	
	...
	else if (hcan->Instance == CAN2) 
		{
			...
			else if (rx_header.StdId == 0x091)
			{	
				L_Gimbal.fri_mode=rx_data[0];
				L_Gimbal.bodan_mode=rx_data[1];
				L_Gimbal.AutoFire_Flag=rx_data[2];
				L_Gimbal.Laser_Flag=rx_data[3];
				L_Gimbal.Gimbal_Flag=rx_data[4];
				L_Gimbal.Freq_state=rx_data[5];
			}
