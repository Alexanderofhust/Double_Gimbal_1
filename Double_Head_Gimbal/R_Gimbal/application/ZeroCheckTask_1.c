...
float ZeroCheck(ZeroCheck_Typedef *Zero,float value,short Zerocheck_mode)
{
	...
	if(Zerocheck_mode==PositionMODE)
		return Zero->ActualValue - Zero->Circle*Zero->CountCycle;
	else if(Zerocheck_mode==SpeedMODE)
...
{
	Gimbal.Pitch.Gyro = IMU_DATA.angle_in_degree[2];
    Gimbal.Pitch.AngularSpeed = IMU_DATA.angular_speed[1];
	Gimbal.Pitch.Motor = ZeroCheck(&ZeroCheck_Pitch,PitchMotorReceive,PositionMODE);
	...
	Gimbal.Yaw.Gyro = IMU_DATA.YawTotalAngle;
	Gimbal.Yaw.AngularSpeed = IMU_DATA.angular_speed[2];
	Gimbal.Yaw.Motor = ZeroCheck(&ZeroCheck_Yaw,YawMotorReceive,PositionMODE);
	Gimbal.Yaw.MotorTransAngle = (Gimbal.Yaw.Motor-Infantry.Yaw_init)/8192.0f*360.0f;
	
	Bodan_Pos=ZeroCheck(&ZeroCheck_BodanPos,BodanReceive.Angle,PositionMODE);
...
void ZeroCheck_Init(void)
{
	...
	ZeroCheck_GyroYaw.LastValue=IMU_DATA.YawTotalAngle;
	
	ZeroCheck_GyroPitch.Circle=0;
	ZeroCheck_GyroPitch.CountCycle=360;
	ZeroCheck_GyroPitch.LastValue=IMU_DATA.angle_in_degree[2];;
